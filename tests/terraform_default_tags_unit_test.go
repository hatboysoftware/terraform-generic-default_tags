...
func TestAzureResources(t *testing.T) {
	...
	tagResult := tagRe.FindStringSubmatch(plan)
	...
	tagCases := []struct {
		...
		{tagResult[1], "foo", tagResult[2], "bar"},
	}

	for _, tc := range tagCases {
		t.Run(tc.gotTag, func(t *testing.T) {
			assert.Equal(t, tc.wantTag, tc.gotTag)
			assert.Equal(t, tc.wantValue, tc.gotValue)
		})
	}
}
